...
def get_network_performance(language_data):
    ...
    char_dict = dict(zip(char_dict, [i for i in range(0, (
        len(char_dict)))]))  # Dictionary where every character key has an integer as value
...
def save_repeated_measures(list_of_results, language_name):
    filename_performance = "Results/Spurt/celex_spurt_performance.csv"
    filename_predictions = "Results/Spurt/celex_spurt_predictions.csv"
...
language_data = pd.read_csv("Data/Processed/celex_format_mono.csv", keep_default_na=False)
language_data = language_data[language_data['englishPOS'] != 'Other']
